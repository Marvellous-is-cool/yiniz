...
function calculateDifficultyFromPerformance(studentResults) {
  ...
  const successRate =
    scores.filter((score) => score >= 0.6).length / scores.length;
...
router.post("/", async (req, res) => {
  const { text, type, subject, correctAnswer, teacherEstimatedDifficulty } =
    req.body;
  ...
  try {
    ...
    const question = await Question.create({
      ...
      mlPredictedDifficulty:
        prediction.difficulty_prediction.predicted_difficulty, // What ML thinks
      predictionConfidence: prediction.difficulty_prediction.confidence,
      status:
        prediction.difficulty_prediction.confidence > 0.7
          ? "ready"
          : "needs_review",
      createdAt: new Date(),
    });
    ...
    res.json({
      ...
      analysis: {
        ...
        agreement:
          teacherEstimatedDifficulty ===
          prediction.difficulty_prediction.predicted_difficulty,
...
async function updateQuestionDifficulty(questionId) {
  try {
    ...
    const adjustedSuccessRate =
      successRate - (avgTime > 180 ? 0.1 : 0) - (variance > 0.3 ? 0.15 : 0);
    ...
    if (
      updatedQuestion.teacherEstimatedDifficulty !== calculatedDifficulty ||
    ...
    console.log(
      `✅ Updated difficulty for question ${questionId}: ${calculatedDifficulty} (confidence: ${confidence.toFixed(
        2
      )})`
    );
  } catch (error) {
...
router.get("/question-insights", async (req, res) => {
  try {
    const questions = await Question.find({}).populate("answers").lean();
    ...
    const insights = questions.map((q) => {
      ...
      return {
        ...
        performance: hasEnoughData
          ? {
              successRate:
                (q.performanceMetrics?.successRate * 100).toFixed(1) + "%",
        ...
        status: {
          ...
          predictionAccurate:
            q.teacherEstimatedDifficulty === q.calculatedDifficulty,
        },
      };
    });
    ...
    const summary = {
      ...
      accuratePredictions: insights.filter((i) => i.status.predictionAccurate)
        .length,
    };
...
router.get("/question/:id/detailed-insights", async (req, res) => {
  ...
  try {
    const performanceData = await fetch(
      `https://yinizai.onrender.com/analyze/question/${questionId}/performance`
    );
  ...
  try {
    ...
    if (recentAnswers.length === 0) {
      ...
      return res.json({
        ...
        reason: "Starting with easy questions",
      });
    }

    // Analyze student performance patterns
    const avgScore =
      recentAnswers.reduce((sum, a) => sum + a.actualScore, 0) /
      recentAnswers.length;
    const avgTime =
      recentAnswers.reduce((sum, a) => sum + a.timeSpent, 0) /
      recentAnswers.length;
    const recentSuccess =
      recentAnswers.slice(0, 3).filter((a) => a.actualScore >= 0.6).length / 3;
    ...
    res.json({
      ...
      adaptive_reasoning: {
        ...
        reason:
          recentSuccess >= 0.8
            ? "Student excelling - increasing difficulty"
            : recentSuccess <= 0.3
            ? "Student struggling - reducing difficulty"
